...
output "ssh_command" {
  ...
  value       = "cloudflared access ssh --hostname ssh-jeanpascal-gerber.nona.company"
}
...
output "ssh_config" {
  ...
  value       = <<-EOT
    Host nexus
      HostName ssh-jeanpascal-gerber.nona.company
      User root
...
output "service_urls" {
  ...
  value = {
    ...
    key => "https://${service.subdomain}-jeanpascal-gerber.nona.company"
  }
}
...
output "secrets" {
  ...
  value = {
    ...
    redpanda_kafka_public_url           = "redpanda-kafka-jeanpascal-gerber.nona.company:9092"
    redpanda_schema_registry_public_url = "http://redpanda-schema-registry-jeanpascal-gerber.nona.company:18081"
